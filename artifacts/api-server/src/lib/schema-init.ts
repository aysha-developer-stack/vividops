...
export async function ensurePushSubscriptionsSchema(): Promise<void> {
  if (pushSubscriptionsSchemaEnsured) return;
...
export async function ensureAllSchemas() {
  ...
  try {
    ...
    await db.execute(sql`
      ...
      CREATE INDEX IF NOT EXISTS active_timer_sessions_job_idx ON active_timer_sessions (job_id);
      CREATE INDEX IF NOT EXISTS active_timer_sessions_heartbeat_idx ON active_timer_sessions (last_heartbeat_at);
    `);
